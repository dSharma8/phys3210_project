...
ax.set_xlim(-r_bh_f*15, r_bh_f*15)
ax.set_ylim(-r_bh_f*15, r_bh_f*15)
...
def reset():
    ...
    global x_cloud, y_cloud, v_x_cloud, v_y_cloud
    global n_particles_consumed, elapsed_time
    x_star, y_star, v_x_star, v_y_star = init_x, init_y, init_v_x, init_v_y
    x_cloud, y_cloud, v_x_cloud, v_y_cloud = (radii * np.cos(theta)) + init_x, (radii * np.sin(theta)) + init_y, np.ones(n_particles) * init_v_x, np.ones(n_particles) * init_v_y
    n_particles_consumed, elapsed_time = 0, 0

mass_v_time = []
for i in range(10):
    reset()
    itcount = 0
    m_star = 10 * (1+0.5*i) * solar_mass # mass of star [kg]
    #m_star = 30 * 1 * solar_mass # mass of star [kg]
    while n_particles_consumed < 1000:
        update(itcount)
    mass_v_time.append([elapsed_time/60, m_star])

print(mass_v_time)  
plt.figure()
times, masses = zip(*mass_v_time)
plt.scatter(times, masses)
plt.show()